...
\subsection{自己位置推定}
   ...
   \begin{eqnarray}
    ...
    R_z = \left[
	   ...
	   \right] 
   \end{eqnarray}
   ここで，ローカル座標系を$(x,y,z)$，グローバル座標系を$(x_g,y_g,z_g)$とすると
   \begin{equation}
    (x_g \  y_g \ z_g) = (x \ y \ z) \cdot R_z \cdot R_x \cdot R_y
   \end{equation}
   となる．これによってローカル座標系の加速度センサで得られた加速度の向きをグローバル座標系に変換することができた．
...
\subsection{障害物回避}
  ...
  \begin{figure}[H]
    \begin{center}
      \includegraphics[width=1.0\hsize]{../../report_2/yamasa/plot/minus_atanh.eps}
    \end{center}
    \caption{$y=-\tanh^{-1}(x-1)$}
    \label{fig::arctanh}
  \end{figure}
